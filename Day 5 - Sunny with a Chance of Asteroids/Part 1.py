...
def int_code_program(int_code):
    ...
    for code in range(0, len(int_code),4) :
        code = int_code[position]
        mode_opcode = str(int_code[position]).zfill(5)
        mode_opcode_list = list(mode_opcode)
        opcode = int(mode_opcode_list[3] + mode_opcode_list[4])
        params_mode = mode_opcode_list[:3] 

        for param_place, mode in enumerate(params_mode.reverse(), start=1):
            param_position = position + param_place

            if opcode == 1:
                if mode == 0:
                    int_code[position + 4] = int_code[int_code[param_position]] + int_code[position + 2]
                if mode == 1:
                    int_code[position + 4] = int_code[param_position] + int_code[position + 2]
        position += 4
            if opcode == 2:
                if mode == 0:
                    int_code[position + 4] =  int_code[int_code[param_position]] * int_code[position + 2]
        ...
        position += 4
            if opcode == 3:
                int_code[int_code[param_position]] = int_code[param_position]
                position += 2
            if opcode == 4:
                int_code[param_position] = int_code[int_code[param_position]]
                position += 2
                break
            if opcode == 99:
                break
...
print(int_code_program(int_code_list))
