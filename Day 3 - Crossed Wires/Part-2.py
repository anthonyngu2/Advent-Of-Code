...
#determined corners of wire from origin
def plot(route):
    origin = [0,0]
    final_cords = []
...
# determine full path
def calc_total_points(route):
    total_route = []
    for x in range(1,len(route),1):
        x1 = route[x-1][0] #line one x start
        y1 = route[x-1][1] #line one y start
        x2 = route[x][0] #line one x end
        y2 = route[x][1] #line one y end
        x_range = abs(x2 - x1)
        y_range = abs(y2 - y1)
...
intersections = [value for value in set_complete_route_1 if value in set_complete_route_2]
#intersections = set(complete_route_1) & set(complete_route_2)

intersections.remove((0,0))

intersections = [list(ele) for ele in intersections]

def total_steps(path1,path2,intersection):
    steps_list = []
    for x in range(len(intersection)):
        steps_1 = len(path1[:path1.index(intersection[x])])
        steps_2 = len(path2[:path2.index(intersection[x])])
        total = steps_1 + steps_2
        steps_list.append(total)
    return steps_list
    
print(min(total_steps(complete_route_1,complete_route_2,intersections)))
